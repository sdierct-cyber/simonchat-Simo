...
// Deterministic builder + editor – returns {ok, text, html?}
...
<body>
  ...
  <div class="wrap">
    <div class="hero">
      <div class="card">
        ...
        <div class="grid3 section">
          ...
          <div class="card"><div class="k">Editable</div><div class="small">Use commands like headline:, cta:, add faq, etc.</div></div>
          <div class="card"><div class="k">Deployable</div><div class="small">Works as standalone HTML.</div></div>
        </div>
      </div>
    ...
    <div class="section card" id="faq">
      <div class="k">FAQ</div>
      <p><strong>How do I edit this?</strong><br/>Use commands like <em>headline: New Title</em>, <em>cta: Join Now</em>, <em>add faq</em>.</p>
      <p><strong>Is this production-ready?</strong><br/>Great starting point — customize freely.</p>
    </div>` : ""}

    <div class="section card">
      <div class="k">Next steps</div>
      <p>Try editing or ask for a new build.</p>
    </div>
  </div>
</body>
</html>`;
...
function parseCommand(input) {
  ...
  if (/^headline\s*:/i.test(low)) return { type: "headline", value: s.split(/:\s*/).slice(1).join(":").trim() };
  if (/^cta\s*:/i.test(low))       return { type: "cta", value: s.split(/:\s*/).slice(1).join(":").trim() };
  if (/^price\s*:/i.test(low))      return { type: "price", value: s.split(/:\s*/).slice(1).join(":").trim().replace(/[^\d.]/g, "") };

  if (/^add\s+faq/i.test(low))         return { type: "toggle", key: "faq", on: true };
  if (/^remove\s+faq/i.test(low))      return { type: "toggle", key: "faq", on: false };
  if (/^add\s+pricing/i.test(low))     return { type: "toggle", key: "pricing", on: true };
  if (/^remove\s+pricing/i.test(low))  return { type: "toggle", key: "pricing", on: false };
  if (/^add\s+testimonials/i.test(low)) return { type: "toggle", key: "testimonials", on: true };
  if (/^remove\s+testimonials/i.test(low)) return { type: "toggle", key: "testimonials", on: false };

  if (/^continue\b/i.test(low)) return { type: "continue" };
...
function applyEdit(html, cmd) {
  ...
  if (cmd.type === "headline" && cmd.value) {
    out = out.replace(
      /(<h1[^>]*id=["']simo_headline["'][^>]*>)([\s\S]*?)(<\/h1>)/i,
      `$1${escapeHtml(cmd.value)}$3`
    );
    out = out.replace(
      /<title>[\s\S]*?<\/title>/i,
      `<title>${escapeHtml(cmd.value)}</title>`
    );
    return out;
  }
  ...
  if (cmd.type === "toggle") {
    ...
    const headlineMatch = out.match(/id=["']simo_headline["'][^>]*>([\s\S]*?)<\/h1>/i);
    const headline = headlineMatch ? headlineMatch[1].replace(/<[^>]+>/g, "").trim() : "Build Faster. Look Better.";

    const ctaMatch = out.match(/id=["']simo_cta["'][^>]*>([\s\S]*?)<\/a>/i);
    const cta = ctaMatch ? ctaMatch[1].replace(/<[^>]+>/g, "").trim() : "Get Started";

    const priceMatch = out.match(/id=["']simo_price["'][^>]*>([\s\S]*?)<\/span>/i);
    const price = priceMatch ? priceMatch[1].replace(/<[^>]+>/g, "").trim() : "29";

    return baseTemplate({ headline, sub: "A clean, modern landing page generated by Simo.", cta, price, faq, pricing, testimonials });
  }
...
exports.handler = async (event) => {
  ...
  if (mode === "build" || cmd.type === "build" || !current.trim()) {
    ...
    if (/fitness|coach|gym|workout/i.test(input)) {
      headline = "Get Fit Without Guesswork";
      sub = "A fitness coaching page you can edit instantly.";
    }
    if (/restaurant|pizza|coffee/i.test(input)) {
      headline = "Your Next Favorite Bite";
      sub = "Restaurant landing page template — fast and clean.";
    }
    if (/app|saas|startup/i.test(input)) {
      headline = "Launch Your SaaS Cleanly";
      sub = "A simple product page with editable sections.";
    }

    const html = baseTemplate({ headline, sub, cta: "Get Started", price: "29", faq: false, pricing: true, testimonials: false });
    return json(200, {
      ok: true,
      text: "Here's your new landing page in the preview.\n\nEdit it with:\n• headline: Your New Title\n• cta: Join Now\n• price: 49\n• add faq\n• remove testimonials\n• add pricing",
      html
    });
  }
