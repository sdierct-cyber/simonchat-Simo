...
(function () {
  ...
  function simo14mTitleCase(value) {
    return String(value || "")
      .replace(/[_-]+/g, " ")
      .replace(/\s+/g, " ")
      .trim()
  ...
  const titleCase = simo14mTitleCase;
  ...
  function simo14fExactLockFromProject(project, fallbackPrompt) {
    ...
    if (simo14fIsRimText(blob)) return { kind: "rim", category: "product", item: "single standalone circular automotive alloy wheel rim", title: "Tire Rim — Product Concept" };
  ...
  function simo14lActiveSubject(project) {
  ...
  function simo14lFollowupPrompt(activeProject, userText, action) {
    const subject = simo14lActiveSubject(activeProject);
    const category = String(activeProject && activeProject.category || activeProject && activeProject.domain || "object").trim();
    const edit = String(userText || "").trim();
    if (!subject || !isVagueEdit(edit) || isClearNewObject(edit)) return edit;
    return [
      `Continue editing the same active design subject: ${subject}.`,
      `Locked category/domain: ${category}.`,
      `User follow-up edit: ${edit}.`,
      "Do not choose a new object. Do not reinterpret the request as a different product.",
      "Preserve the exact object family, silhouette, structure, and purpose.",
      "Only change the requested color, finish, material, accent, lighting, style, feature, or design detail.",
      "Show the closest updated real visual first."
    ].join(" ");
  }
  ...
  function statusTextForAction(action, category) {
    ...
    const c = simo14mTitleCase(category || "design");
...
function smartShortTitle(value, fallback) {
  ...
  return simo14mTitleCase(clipped || "Visual Concept");
}
...
function classifyPrompt(prompt) {
  ...
  if (has(/\b(dog leash|leash|collar|pet accessory|mouse|computer mouse|wireless mouse|bottle|package|packaging|box|jar|can|rim|rims|wheel|wheels|tire rim|alloy wheel|phone case|toy|tool|device|accessory|product|lamp|appliance|book cover|book jacket|poster|flyer|brochure|menu)\b/)) {
    { const productItem = item("product concept"); const productTitle = productItem ? `${toTitleCase(productItem)} Design` : "Product / Accessory Concept"; return { category: "product", item: productItem, title: productTitle, kind: "product" }; }
  }
...
function seededRealImageUrl(project) {
  ...
  function mockVisualSvg(project, options = {}) {
    ...
    if (cat === "product" || cat === "object" || cat === "industrial" || cat === "wearable" || cat === "furniture") {
      const isBottle = /bottle/i.test(item);
      const isLamp = /(light|lamp|fixture)/i.test(item);
      const isChair = /(chair|stool|sofa|desk|table)/i.test(item);
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1400" height="900" viewBox="0 0 1400 900">
        <defs>
  ...
  function buildProject(prompt, action, overrides) {
    ...
    const project = {
      ...
      controls: controlsFor(seed.category),
      ...
      isFollowupEdit: useActive,
      followupSubjectLock: useActive ? (active.lockedSubject || active.exactSubject || active.requestedAsset || active.item || active.title || "") : "",
      updatedAt: new Date().toISOString(),
      phase: PHASE,
      action: action || "base",
      ...(overrides || {}),
  ...
  function safeProjectForCard(project) {
    ...
    return {
      ...
      controls: Array.isArray(p.controls) ? p.controls : controlsFor(p.category || "object"),
      imageUrl: p.imageUrl || "",
      wants3D: !!p.wants3D,
  ...
  function renderRealImageRequired(project, message) {
    ...
    const safeCat = esc(simo14mTitleCase(project?.category || "design"));
  ...
  function openConnectedWorkspace(project, sourcePrompt, reason) {
    ...
    const title = simo14mTitleCase(project?.item || sourcePrompt || "Design Concept");
  ...
  async function open3D(project, sourcePrompt) {
    ...
    try {
      activeProject.wants3D = true;
      activeProject.last3dAsset = {
        title: `${displayTitle(activeProject)} connected design workspace`,
        url: "connected-design-workspace",
        source_label: "Simo visual-first workspace",
        exact: false,
      };
      setActive(activeProject);

      // PHASE 10.6B: The button must visibly open a workspace immediately.
      // Do NOT wait for /api/real-3d-assets here, because that made the click feel like nothing changed.
      // The image remains the source of truth; true 3D model routing can be added later as a second step.
      openConnectedWorkspace(
        activeProject,
        promptText,
        "This is the visible rotate/design workspace for the exact visual concept. Simo is not showing a random robot, astronaut, or unrelated fallback model. The current image stays as the source of truth while the user refines toward real 3D/design output."
      );

      addAssistant(renderProject(
        activeProject,
        "Opened the connected rotate/design workspace for this exact concept. The image stays as the source of truth; use the workspace controls to refine the design before we connect true object-specific 3D assets."
      ));
      return true;
    } catch (err) {
      console.warn("Simo visual workspace open failed:", err);
      addAssistant(renderProject(
        activeProject,
        `Simo stayed in ${activeProject.category || "design"} mode, but the workspace did not open on this try. The visual concept is still active for editing.`
      ));
      return true;
    }
  }
  ...
  async function run(prompt, action = "base") {
    ...
    try {
      ...
      let apiData = null;
      try {
        const requestPrompt = project.isFollowupEdit
          ? simo14lFollowupPrompt(project, effectiveText, action)
          : effectiveText;
        apiData = await api("/api/generate-visual", {
          ...
          followup_subject_lock: !!project.isFollowupEdit,
          followup_edit_lock: false,
          active_image_url: project.imageUrl || "",
          ...
          visual_core_phase: "14L",
          refinement_strength: actionIsMeaningful(action) ? "subject-locked-followup" : "base-visual-first",
  ...
  function submitCapture(forceText) {
    ...
    let shouldRun = true;
    try {
      shouldRun = isDesignPrompt(text);
    } catch (err) {
      console.warn("[SIMO] PHASE 14M-R3 isDesignPrompt failed; allowing submit:", err);
      shouldRun = true;
    }
