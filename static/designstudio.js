...
/* ================================
   PHASE 4.1 SAFE BRIDGE APPEND
   Design Studio -> Builder payload bridge
   Append-only block for static/designstudio.js
================================ */
(() => {
  if (window.__SIMO_PHASE41_SAFE_BRIDGE__) return;
  window.__SIMO_PHASE41_SAFE_BRIDGE__ = true;
  ...
  function getActiveStudioType() {
    const activePanel =
      $('[data-ds-panel].active') ||
      $('[data-ds-panel][aria-hidden="false"]') ||
      $('[data-ds-panel]');

    if (!activePanel) return "design concept";

    return (
      activePanel.getAttribute("data-ds-panel") ||
      activePanel.dataset.dsPanel ||
      "design concept"
    );
  }
  ...
  function getOutputModeLabel() {
    const checked =
      $('input[name="dsOutputMode"]:checked') ||
      $('input[name="outputMode"]:checked') ||
      $('input[type="radio"][name*="mode"]:checked');

    if (!checked) return "Text Mode";

    const raw = (checked.value || checked.id || "").toLowerCase();

    if (raw.includes("visual")) return "Visual Mode";
    return "Text Mode";
  }

  function getDesignStudioOutputText() {
    const candidates = [
      "#designStudioOutput",
      "#dsOutput",
      "[data-ds-output]",
    ...
    for (const sel of candidates) {
      ...
      if (text && !/nothing generated yet/i.test(text)) {
        return text;
      }
    }
  ...
  function buildBuilderPayload(sourceText, studioType, outputMode) {
    const cleanType = String(studioType || "design concept")
      .replace(/[-_]+/g, " ")
      .trim();

    return [
      "[SIMO_BUILDER]",
      `DESIGN_STUDIO_TYPE: ${cleanType}`,
      `OUTPUT_MODE: ${outputMode}`,
      "",
      "Create a polished, premium, builder-ready result based on the concept below.",
      "",
      "REQUIREMENTS:",
      "- Build a real usable result, not notes about the result.",
      "- Make it feel premium, modern, cohesive, and visually polished.",
      "- Use clear structure, clean hierarchy, strong spacing, and strong sections.",
      "- Do not explain what you would build. Build it directly.",
      "- Do not repeat the concept back unnecessarily.",
      "- Make the output feel like a finished direction, not a rough draft.",
      "",
      "SOURCE_CONCEPT:",
      sourceText.trim()
    ].join("\n");
  ...
  function showToast(message, type = "info") {
    if (typeof window.toast === "function") {
      window.toast(message, type, 2600);
      return;
    }
    console.log(`[${type}] ${message}`);
  }

  async function handleGenerateSendBridge() {
    // Let existing generate logic finish first
    await new Promise((resolve) => setTimeout(resolve, 80));

    const outputText = getDesignStudioOutputText().trim();
    if (!outputText) {
      showToast("Generate a concept first so Simo has something to build from.", "error");
      return;
    }

    const studioType = getActiveStudioType();
    const outputMode = getOutputModeLabel();
    const payload = buildBuilderPayload(outputText, studioType, outputMode);

    const injected = injectIntoChat(payload);
    if (!injected) {
      showToast("Could not find the chat input.", "error");
      return;
    }

    const sent = clickSend();
    if (!sent) {
      showToast("Builder payload prepared in chat, but Send button was not found.", "info");
      return;
    }

    showToast("Sent to Builder.", "success");
  }

  document.addEventListener(
    "click",
    (event) => {
      const trigger = event.target.closest("[data-ds-generate-send]");
      if (!trigger) return;

      // Do not block existing logic — just enhance it safely
      handleGenerateSendBridge().catch((err) => {
        console.error("PHASE 4.1 safe bridge error:", err);
        showToast("Design Studio bridge hit an error.", "error");
      });
    },
    true
  );
